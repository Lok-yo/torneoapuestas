...
export default function LiveBetTicker() {
  ...
  useEffect(() => {
    ...
    listMarkets()
      .then((markets) => {
        if (cancelled || !Array.isArray(markets) || markets.length === 0) return
  ...
  return (
    <div className="bento overflow-hidden p-3 hover:border-violet-500/30" role="region" aria-label="Cinta de mercados en vivo">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="ticker-track gap-3 pr-3">
          {activities.map((act) => (
            <TickerChip key={act.id} act={act} />
          ))}
          {/* Duplicado visual para animación continua, oculto a lectores de pantalla */}
          <div className="flex gap-3 shrink-0" aria-hidden="true">
            {activities.map((act, i) => (
              <TickerChip key={`${act.id}-dup-${i}`} act={act} />
            ))}
          </div>
        </div>
      </div>

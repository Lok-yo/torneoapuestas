  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-2xl border border-violet-500/30 bg-gradient-to-br from-violet-950/30 via-zinc-900/60 to-zinc-950 p-6">
        ...
        <p className="text-xs text-zinc-500">FPMM: {market.fpmm}</p>
      </div>
      ...
      {market.state !== MARKET_STATE.ACTIVE ? (
        ...
        <form onSubmit={handleBuy} className="flex flex-col gap-4 rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6">
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => setOutcomeIndex(0)}
              className={`flex-1 rounded-xl py-2.5 text-sm font-semibold ${outcomeIndex === 0 ? 'bg-emerald-600 text-white' : 'border border-zinc-800 text-zinc-300'}`}
            >
              Outcome A
            </button>
            <button
              ...
              className={`flex-1 rounded-xl py-2.5 text-sm font-semibold ${outcomeIndex === 1 ? 'bg-rose-600 text-white' : 'border border-zinc-800 text-zinc-300'}`}
            >
              Outcome B
            </button>
          </div>
          ...
          <p className="text-center text-[11px] text-zinc-600">Wallet conectada: {address}</p>
        </form>
      )}
    </div>
  )

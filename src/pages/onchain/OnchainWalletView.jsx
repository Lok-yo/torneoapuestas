...
export default function OnchainWalletView() {
  ...
  if (!isConnected) {
    return (
      <div className="mx-auto flex max-w-md flex-col items-center gap-4 rounded-3xl border border-violet-500/20 bg-gradient-to-br from-violet-500/10 via-zinc-900 to-zinc-950 p-8 text-center">
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="rounded-3xl border border-violet-500/20 bg-gradient-to-br from-violet-500/10 via-zinc-900 to-zinc-950 p-6">
        ...
        <p className="mt-1 text-4xl font-bold text-zinc-50">{formatUsdc(usdcBalance)} USDC</p>
      </div>
          ...
          <div className="overflow-x-auto rounded-2xl border border-zinc-800">
            <table className="w-full text-sm">
              <thead className="bg-zinc-900/80 text-left text-xs uppercase tracking-wide text-zinc-500">
                <tr>
                  <th scope="col" className="px-4 py-3 font-medium">Mercado (condition)</th>
              ...
              <tbody className="divide-y divide-zinc-800">
                {positions.map((p) => (
                  <tr key={`${p.condition_id}-${p.position_id}`} className="hover:bg-zinc-900/30">
                    ...
                    <td className="px-4 py-3 text-right text-zinc-200">{p.balance}</td>
                  </tr>
                ))}
              </tbody>
            </table>

...
- [ ] A user can Google sign in, atomically claim a case-insensitive username, register, pass roster freeze, and see tournament, leaderboard, and history after an authorized result. **CRITICAL gap found by `sdd-verify` (see `verify-report.md`), not previously flagged by any apply batch or mid-apply validator: unchecked.** Every step from registration onward (registration, roster freeze, bracket, official result, leaderboard/history) has dedicated, passing Playwright coverage — sign-in + username claim (`e2e/auth-onboarding.spec.js`), registration/freeze/bracket/result (`e2e/tournament-flow.spec.js`), leaderboard/history (`e2e/leaderboard-history.spec.js`) — and atomic case-insensitive username uniqueness is proven at the database layer (`username_claim_race.sql`). **But no application-level path exists to create a tournament in the first place**: no RPC, Edge Function, repository function, UI route, or migration/seed ever inserts a `tournaments` row in production code — every test either inserts one as a raw superuser fixture or stubs the network response. `design.md`'s own file-changes table planned a `tournament-command` Edge Function that was never built. So this chain is only genuinely reachable end-to-end from an already-seeded database today, not from zero in a live deployment. **Deliberately deferred, not fixed in this change** (maintainer decision 2026-08-12): documented here and in `tasks.md`'s "Deferred to follow-up" section for a future contributor to implement (a minimal `create_tournament` command is the missing piece — see that section for the exact scope).

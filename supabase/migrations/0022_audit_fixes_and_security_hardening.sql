-- 0022_audit_fixes_and_security_hardening.sql
-- Fixes security & accounting gaps identified in PDF audit:
--
-- 1. resolve_market: Enforces tournament/market ownership. An organizer can ONLY
--    resolve markets for tournaments they own (tournaments.organizer_id = auth.uid())
--    or markets they created (markets.created_by = auth.uid()). Admins can resolve any.
--
-- 2. buy_market_shares: Recalculates weighted average price (avg_price) on position
--    upsert when additional shares are purchased:
--    new_avg_price = ((old_shares * old_avg_price) + (new_shares * buy_price)) / total_shares.

-- =====================================================================
-- 1. Fix resolve_market ownership check
-- =====================================================================
...
BEGIN
  ...
  -- Validate ownership: Caller must be admin, creator of the market, OR organizer of the linked tournament
  IF v_is_admin THEN
    v_is_owner := true;
...
-- =====================================================================
-- 2. Fix buy_market_shares weighted avg_price calculation
-- =====================================================================
